...
const Login = () => {
  ...
  <div class="p-2 w-full md:w-1/3">
        ...
        <div class="p-2 w-1/3">
          <button onClick={login} class="flex mx-auto text-white bg-green-600 border-0 py-2 px-8 focus:outline-none hover:bg-green-700 rounded text-lg">{loading?<TailSpin height={25} color="white"/>:'Login'}</button>
        </div>

        <div>Do not have Account <Link to='/signup'><span className='text-blue-500'> Sign Up</span></Link></div>
  </div>
  )
}
